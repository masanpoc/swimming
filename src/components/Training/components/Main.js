      ...
      {mainSets.every((el) => typeof el == "string") ? (
        ...
        <div className="flex flex-col space-y-3">
          {mainSets.map((blockSet, index) => {
            return (
              <div
                className="flex w-full pt-4 px-6 justify-between"
                key={`${blockSet}-${index}`}
              >
                <div className="flex space-x-1 pt-1" style={{ width: "20%" }}>
                  ...
                  <div className="h-full w-full flex flex-col space-y-12 ">
                    {blockSet.eachSet.map((singleSet, i) => {
                      eachSetMetersList[index + 1].push(
                        singleSet.split("x")[1]
                      );
                      return (
                        <h2 key={singleSet + i} className="w-full text-sm">
                          {singleSet}
                        </h2>
                      );
                    })}
                    ...
                    if (index == 0) {
                      ...
                      return (
                        <h3 className="text-left" key={main[index2].id}>
                          ...
                          {generateTimes(
                            pace,
                            Number(eachSetMetersList[index + 1][index2]),
                            main[index2].stroke
                          )}
                        </h3>
                      );
