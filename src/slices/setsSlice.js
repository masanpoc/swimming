...
export const setsReducer = createSlice({
  ...
  initialState: {
    warmup: { total: 0, each: [] },
    technique: { total: 0, each: [], sets: [] },
    main: { total: 0, each: [], sets: [] },
    cooldown: { total: 0, each: [] },
  },
  reducers: {
        ...
        if (eachList.length > 3) {
          if (Math.random() < 0.5) {
            ...
            let setList = [
              {
                sets: 2,
                eachSet: eachList1.map((el) => generateTrainingSet(el / 2)),
              },
              {
                sets: 2,
                eachSet: eachList2.map((el) => generateTrainingSet(el / 2)),
              },
            ];
            ...
            let setList = [
              {
                sets: 2,
                eachSet: eachList.map((el) => generateTrainingSet(el / 2)),
              },
            ];
            state.main.sets = setList;
          }
