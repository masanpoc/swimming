...
export default function generateTotal(meters) {
  ...
  if (ref <= 5 && totalsForConditionalSet.every((el) => el < 600)) {
    ...
    let randomIndexToAddMeters = Math.floor(
      Math.random() * arrToCorrect.length
    );
    let toAdd = 0;
  ...
  return {
    warmup: { total: arrCorrected[0][0] },
    technique: { total: arrCorrected[1][0] },
    main: { total: arrCorrected[2][0] },
    cooldown: { total: arrCorrected[3][0] },
  };
}
